(function ($) {
 ...
 ===========================================*/
    ...
    function toggleClearFilterButton() { //toggle clear filter button if any of the input field has value set
        if (typeof ($(".collection-search-form input.filter-input").val()) !== "undefined") {
            if ($(".collection-search-form input.filter-input").val().length != 0 || $(".collection-search-form input.filter-input").is(':checked')) { // if any of the filter input has value or any of checkbox from checkbox list is checked
                console.log("show");
                $(".collection-search-form #clear-filter-btn").show();
            } else { // if not hide the clear filter button
    ...
    function setCheckboxLabel(element_key) { // set to show/display the selected options from the check list
        ...
        if (typeof selected !== 'undefined' && selected.length > 0) {
            ...
            $(".check-title." + element_key + "-title").html(element_key);
        }
    }
