		...
		public function importXmlCollectionListings() {
			...
			for ( $i = $offset; $i < ( $offset + $limits ); $i ++ ) {
				...
				if ( isset( $xml_data["german"][ $i ] ) ) {
					...
					$result = $this->insertUpdateSinlgeCollection( $row_gr, "de", 0 ); //insert/update german version of collection post
...
//					error_log( print_r( $original_id, true ) );
            ...
            update_post_meta( $post_id, "collection_title", is_string( $data["title"] ) ? $data["title"] : "" );
			...
			update_post_meta( $post_id, "creditline", is_string( $data["creditline"] ) ? $data["creditline"] : "" );
			update_post_meta( $post_id, "invnr_number", ! empty( $invnr ) ? $invnr : "" );
			update_post_meta( $post_id, "invnr_cat", ! empty( $invnr ) ? (int) $invnr : "" );
...
//				}
		...
		function preprocessXmlRowData(
			...
			$arr["creditline"]  = isset( $row["content"]["creditline"] ) ? $row["content"]["creditline"] : "";
